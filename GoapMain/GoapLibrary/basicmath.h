...
public:
    ...
    OutputType similarity() const { return 1 - _distance; } // 0 means equal
    OutputType distance() const { return _distance; }

    /// Gives an idea of the magnitude of the vector
    OutputType aModule() const { return _aModule; }
    OutputType bModule() const { return _bModule; }

    /// Offset to add to all components to be centered
    OutputType aShift() const { return _aShift; }
    OutputType bShift() const { return _bShift; }
